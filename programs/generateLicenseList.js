const crawler = require("npm-license-crawler"), http = require("http"),
...
const meteorPackages = {
  meteor : {
    licenses : "MIT",
    licenseUrl :
        "https://raw.githubusercontent.com/meteor/meteor/devel/LICENSE",
  },
  "alanning:roles" : {
    licenses : "MIT",
    licenseUrl :
...
function downloadToStream(project, url, licenseId, originalLicenseUrl) {
  ...
  if (url.includes("raw")) {
    const SPDXUrl =
        `https://github.com/spdx/license-list/raw/master/${licenseId}.txt`;
...
crawler.dumpLicenses({start : [ "." ]}, (error, res) => {
  ...
  const streams = Object.keys(res).map(
      (project) => downloadToStream(project, res[project].licenseUrl,
                                    res[project].licenses),
  );
  ...
  getSortedKeys(licenseCount)
      .forEach(
          (license) => console.log(`${license}   ${licenseCount[license]}`),
      );
  console.log("********************");
});
