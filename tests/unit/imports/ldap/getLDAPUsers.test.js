...
//   ldapjs: ldap,
// });
const getLDAPUsers = rewiremock.proxy("#root/imports/ldap/getLDAPUsers.js", {
  ldapjs : ldap,
});
...
describe("getLDAPUsers", function() {
  ...
  describe("legacy inactive user detection settings", function(done) {
    let client;
    // skipcq: JS-0241
    beforeEach(function() { ldap.createClient.reset(); });
    ...
    // skipcq: JS-0241
    it("adds property map attributes to allowlist automatically",
       function(done) {
  ...
  describe("inactive user detection strategy: UAC", function() {
    ...
    for (let i = 0; i < 32; ++i) {
      ...
      it(
          `uAC property set to ${
              value}: returns user object with isInactive === ${
              expectedResult}`,
          generateTestCase(value, expectedResult),
      );
    }
