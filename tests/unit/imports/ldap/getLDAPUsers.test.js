...
const getLDAPUsers =
    proxyquire('../../../../imports/ldap/getLDAPUsers', {'ldapjs' : ldap});
...
describe('getLDAPUsers', function() {
  ...
  describe('legacy inactive user detection settings', function(done) {
    ...
    beforeEach(function() { ldap.createClient.resetHistory(); });
    ...
    it('returns user object with isInactive property set to true',
       function(done) {
         let client = {
         ...
         getLDAPUsers(settings)
             ...
             .catch((error) => { done(error2); });
       });

    it('adds property map attributes to whitelist automatically',
       function(done) {
         const s = Object.assign({}, settings, {
           propertyMap :
               {username : 'someweirdAttribute', email : 'anEmailAttribute'},
             ...
             .then((result) => {
               try {
                 ...
                 expect(options.attributes).to.include.members([
                   'someweirdAttribute', 'anEmailAttribute'
                 ]);
                 done();
               } catch (error) {
                 done(error);
    ...
    it('returns user object with isInactive property set to false',
       function(done) {
         let client = {
  ...
  describe('inactive user detection strategy: none', function(done) {
    ...
    it('uses the none strategy if an invalid strategy is given',
       function(done) {
  ...
  describe('inactive user detection strategy: UAC', function() {
    ...
    const activeUsers = false, inactiveUsers = true,
            ...
            return function(done) {
              ...
              client = {
                search : asyncStubs.returns(
                    2, ldapSearchResponseWithGivenUACValue(value)),
                unbind : asyncStubs.returnsError(0, 'Some error')
              };
              ldap.createClient.returns(client);
              ...
              getLDAPUsers(settings)
                  .then((result) => {
                    try {
                      expect(result.users[0].isInactive)
    ...
    for (let i = 0; i < 32; ++i) {
      ...
      it(`uAC property set to ${
             value}: returns user object with isInactive === ${expectedResult}`,
         generateTestCase(value, expectedResult));
    }
  });
  ...
  describe('inactive user detection strategy: property', function() {
    ...
    it('returns isInactive == true if given property is set to requested value',
       function(done) {
         const settings = {
         ...
         client = {
           search : asyncStubs.returns(
               2, ldapSearchResponseWithGivenAttribute('active', '1')),
           unbind : asyncStubs.returnsError(0, 'Some error')
         };
         ldap.createClient.returns(client);
    ...
    it('returns isInactive == false if given property is not set to requested value',
       function(done) {
         const settings = {
           inactiveUsers : {strategy : 'property', properties : {active : '1'}}
         };
         ...
         client = {
           search : asyncStubs.returns(
               2, ldapSearchResponseWithGivenAttribute('active', '2')),
           unbind : asyncStubs.returnsError(0, 'Some error')
         };
         ldap.createClient.returns(client);
