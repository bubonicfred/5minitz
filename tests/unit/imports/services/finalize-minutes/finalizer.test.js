...
let ms1 = {getMailLanguage : () => { return 'en'; }};
...
let i18n = {
  ...
  runWithLocale : (locale, callback) => { callback(); },
};
...
const {Finalizer} =
    proxyquire('../../../../../imports/services/finalize-minutes/finalizer', {
      ...
      '/imports/helpers/promisedMethods' :
          {PromisedMethods, '@noCallThru' : true},
      '/imports/mail/FinalizeMailHandler' :
          {FinalizeMailHandler, '@noCallThru' : true},
...
describe('Finalizer', function() {
  ...
  describe('#finalize', function() {
    ...
    it('sends the id to the meteor method workflow.finalizeMinute', function() {
      ...
      expect(Meteor.call.calledWith('workflow.finalizeMinute', minutesId))
          .to.be.true;
    });
    ...
    it('sends the id to the meteor method documentgeneration.createAndStoreFile',
       function() {
         ...
         expect(Meteor.call.calledWith('documentgeneration.createAndStoreFile',
                                       minutesId))
             .to.be.true;
       });
  });
  ...
  describe('#unfinalize', function() {
    ...
    it('sends the id to the meteor method workflow.unfinalizeMinute',
       function() {
         ...
         expect(Meteor.call.calledWithExactly('workflow.unfinalizeMinute',
                                              minutesId))
             .to.be.true;
       });
    ...
    it('sends the id to the meteor method documentgeneration.removeFile',
       function() {
         ...
         expect(Meteor.call.calledWithExactly('documentgeneration.removeFile',
                                              minutesId))
             .to.be.true;
       });
  });
  ...
  describe('#finalizedInfo', function() {
    ...
    afterEach(function() { MinutesSchema.findOne.resetHistory(); });
