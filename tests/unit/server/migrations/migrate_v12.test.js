...
let MinutesFinder = {
  ...
  firstMinutesOfMeetingSeries() { return this.firstMinutesResult; },
  nextMinutesResult : {},
  nextMinutes(minutes) { return this.nextMinutesResult[minutes._id]; },
  previousMinutesResult : {},
  previousMinutes(minutes) { return this.previousMinutesResult[minutes._id]; }
};
class MeteorError {}
...
const {MigrateV12} = proxyquire('../../../../server/migrations/migrate_v12', {
  ...
  '/imports/collections/meetingseries.schema' :
      {MeetingSeriesSchema, '@noCallThru' : true},
...
describe('Migrate Version 12', function() {
  ...
  beforeEach(function() {
    sndFakeMinute = {
      ...
      topics : [
        ...
        },
        {
          _id : '#T02',
          infoItems :
              [ {_id : '#I03', details : [ {text : 'd5'} ]}, {_id : '#I04'} ]
        }
      ]
    };
    ...
    MeetingSeriesSchema.find = () => { return [ fakeMeetingSeries ]; };

    MinutesSchema.find = () => { return [ firstFakeMinute, sndFakeMinute ]; };
  });
  ...
  describe('#up', function() {
    ...
    it('sets the correct createdInMinute-attribute for all details in minutes and meetingSeries',
       function() {
         ...
         expect(sndFakeMinute.topics[0].infoItems[0].details[0].createdInMinute)
             .to.equal(FIRST_MIN_ID);
         expect(fakeMeetingSeries.topics[0]
                    .infoItems[0]
                    .details[0]
                    .createdInMinute)
             .to.equal(FIRST_MIN_ID);
         expect(fakeMeetingSeries.openTopics[1]
                    .infoItems[0]
                    .details[0]
                    .createdInMinute)
             .to.equal(FIRST_MIN_ID);
         // detail created in 2nd minute => createdInMinute = 2nd Minute
         expect(sndFakeMinute.topics[1].infoItems[0].details[0].createdInMinute)
             .to.equal(SND_MIN_ID);
         expect(fakeMeetingSeries.topics[1]
                    .infoItems[0]
                    .details[0]
                    .createdInMinute)
             .to.equal(SND_MIN_ID);
         expect(fakeMeetingSeries.openTopics[0]
                    .infoItems[0]
                    .details[0]
                    .createdInMinute)
             .to.equal(SND_MIN_ID);
         ...
         expect(sndFakeMinute.topics[0].infoItems[0].details[2].createdInMinute)
             .to.equal(SND_MIN_ID);
       });
    ...
    it('sets the correct _id attribute for all details in minutes and meetingSeries',
       function() {
         ...
         expect(sndFakeMinute.topics[0].infoItems[0].details[0]._id)
             .to.equal(detailIdInFirstMinute);
         expect(fakeMeetingSeries.topics[0].infoItems[0].details[0]._id)
             .to.equal(detailIdInFirstMinute);
         expect(fakeMeetingSeries.openTopics[1].infoItems[0].details[0]._id)
             .to.equal(detailIdInFirstMinute);
       });
  });
  ...
  describe('#down', function() {
    beforeEach(function() {
      let addCreatedInMinuteFakeAttribute =
          (topic) => { topic.createdInMinute = 'fakeID'; };
