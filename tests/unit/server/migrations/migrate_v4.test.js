...
describe('MigrateV4', function() {
  ...
  let minutesDoc, topicDocWithResponsible, topicDocWithoutResponsible,
      meetingSeriesDoc, topicDocWithRespOfSeriesTopics,
      topicDocWithoutRespOfSeriesTopics, topicDocWithRespOfSeriesOpenTopics,
  ...
  beforeEach(function() {
    ...
    meetingSeriesDoc = {
      _id : 'series#1',
      topics :
          [ topicDocWithoutRespOfSeriesTopics, topicDocWithRespOfSeriesTopics ],
      openTopics : [
        topicDocWithoutRespOfSeriesOpenTopics,
    ...
    topicsWithResponsible = [
      topicDocWithResponsible, topicDocWithRespOfSeriesTopics,
      topicDocWithRespOfSeriesOpenTopics
    ];
    topicWithoutResponsible = [
      topicDocWithoutResponsible, topicDocWithoutRespOfSeriesTopics,
      topicDocWithoutRespOfSeriesOpenTopics
    ];
  });
  ...
  describe('#up', function() {
    it('converts the responsible string of each topics inside every minutes to an array containing the responsible',
       function() {
         MigrateV4.up();
         topicsWithResponsible.forEach(function(
             topicDoc) { expect(topicDoc.responsibles).to.have.length(1); });
         topicWithoutResponsible.forEach(function(
             topicDoc) { expect(topicDoc.responsibles).to.have.length(0); });
       });
    ...
    it('sends the series id as selector to the update call on the meeting series collection',
       function() {
         MigrateV4.up();
         expect(MeetingSeriesSchema.update.calledWith(meetingSeriesDoc._id))
             .to.be.true;
       });
  });
