...
let MinutesFinder = {
  ...
  nextMinutesResult : {},
  nextMinutes(minutes) {
    console.log(`finding next minutes of ${minutes._id}: ${
...
describe('Migrate Version 10', function() {
  ...
  describe('#up', function() {
    let checkTopicHasProperty =
        topic => { expect(topic).to.have.ownProperty('createdInMinute'); };
    ...
    it('sets the correct id for the createdInMinute-attribute', function() {
      ...
      expect(fakeMeetingSeries.topics[0].createdInMinute)
          .to.equal(FIRST_MIN_ID);
      expect(fakeMeetingSeries.openTopics[1].createdInMinute)
          .to.equal(FIRST_MIN_ID);
      ...
      expect(fakeMeetingSeries.openTopics[0].createdInMinute)
          .to.equal(SND_MIN_ID);
    });
  });
  ...
  describe('#down', function() {
    ...
    it('removes the createdInMinute-attribute', function() {
      ...
      let checkTopicHasNoAttribute =
          topic => { expect(topic).not.have.ownProperty('createdInMinute'); };
