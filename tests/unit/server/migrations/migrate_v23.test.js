...
describe('MigrateV23', function() {
  describe('#up', function() {
    ...
    it('calls update method for every topic in the topic collection which has no responsibles set',
       function() {
         const topics =
             [ topicWithResponsiblesNull, topicsWithResponsiblesDefined ];
         TopicSchemaCollection.find.returns(topics);

         MigrateV23.up();

         const expectedNumberOfCallsToUpdate = 1;
         expect(TopicSchemaCollection.update.callCount)
             .to.equal(expectedNumberOfCallsToUpdate);
       });
    ...
    it('converts null in topic collection entries to an empty array',
       function() {
         ...
         expect(TopicSchemaCollection.update.calledWithExactly(undefined, {
           $set : {responsibles : []}
         })).to.be.true;
       });
    ...
    it('calls the update method for every minutes which contains at least one topic where the responsibles field is not an array',
       function() {
         ...
         const expectedNumberOfCallsToUpdate = 1;
         expect(MinutesSchemaCollection.update.callCount)
             .to.equal(expectedNumberOfCallsToUpdate);
       });
    ...
    it('converts null in topic collection entries to an empty array',
       function() {
         ...
         expect(MinutesSchemaCollection.update.calledWithExactly(undefined, {
