...
describe("Migrate Version 20", function() {
  ...
  beforeEach(function() {
    ...
    TopicSchema.find = () => { return [ fakeTopic ]; };
  });
  ...
  describe("#up", function() {
    let checkDetailHasProperty =
        (detail) => { expect(detail).to.have.ownProperty("isNew"); };
    ...
    it("sets the correct isNew-attribute for all details in minutes",
       function() {
         MigrateV20.up();
         expect(firstFakeMinute.topics[0].infoItems[0].details[0].isNew)
             .to.equal(
                 true,
             );
         expect(firstFakeMinute.topics[0].infoItems[1].details[0].isNew)
             .to.equal(
                 true,
             );
         expect(firstFakeMinute.topics[0].infoItems[1].details[1].isNew)
             .to.equal(
                 true,
             );
         expect(firstFakeMinute.topics[1].infoItems[0].details[0].isNew)
             .to.equal(
                 true,
             );

         expect(sndFakeMinute.topics[0].infoItems[0].details[0].isNew)
             .to.equal(
                 false,
             );
         expect(sndFakeMinute.topics[0].infoItems[0].details[1].isNew)
             .to.equal(
                 true,
             );
       });
  ...
  describe("#down", function() {
    beforeEach(function() {
      let addIsNewToDetail = (topic) => {
        topic.infoItems.forEach((infoItem) => {
          if (infoItem.details) {
            infoItem.details.forEach((detail) => { detail.isNew = true; });
          }
        });
      };
    ...
    it("removes the isNew attribute", function() {
      MigrateV20.down();
      let checkDetailHasNoProperty =
          (detail) => { expect(detail).not.have.ownProperty("isNew"); };
