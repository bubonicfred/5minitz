...
describe("MinutesGenerator", function() {
  describe("#generate", function() {
    ...
    it("should generate the correct amount of minutes", function() {
      ...
      expect(generator.generate(TOPICS_GENERATOR))
          .to.have.length(
              config.minutesCount,
          );
    });
    ...
    it("should set all minutes as finalized except the last one", function() {
      ...
      res.forEach((m, i) => {
        ...
        if (!isLastOne) {
          expect(m.isFinalized, "not-last element should be finalized")
              .to.be.true;
        } else {
          expect(m.isFinalized, "last element should not be finalized")
              .to.be.false;
        }
      });
    });
  ...
  describe("#_tickOneDay", function() {
    it("should increase the next minutes date by one day", function() {
      ...
      expect(MinutesGenerator._formatDate(generator.nextMinutesDate))
          .to.equal(
              "2016-08-27",
          );
    });
