...
describe('Attachments', function() {
  ...
  const getNewMeetingName =
      () => { _meetingCounter++ return _meetingNameBase + _meetingCounter }
  ...
  before('reload page and reset app', function() {
    ...
    _localPublicDir = server.call(
        'e2e.getServerCurrentWorkingDir') // call meteor server method
    _localPublicDir +=
        '/../web.browser/app/' // location of files from "/public" directory
    _staticLocalFilename = _localPublicDir + 'favicon.ico'
  })

  beforeEach(
      'goto start page and make sure test user is logged in', function() {
        ...
        _lastMinutesID =
            E2EMinutes.addMinutesToMeetingSeries(_projectName, _lastMeetingName)
      })
  ...
  it('can upload an attachment to the server (as moderator)', function() {
    ...
    const attachment = E2EAttachments.getAttachmentDocsForMinuteID(
        _lastMinutesID)[0] const serverAttachmentDir =
        server.call('e2e.getServerAttachmentsDir')
    const serverAttachmentFilename = serverAttachmentDir + '/' +
                                     _lastMeetingSeriesID + '/' +
                                     attachment._id + '.' + attachment.extension
    expect(fs.existsSync(serverAttachmentFilename),
           'Attachment file should exist on server: ' +
               serverAttachmentFilename)
        .to.be.ok
    ...
    expect(md5local, 'Local file should have same MD5 checksum as server file')
        .to.equal(md5server)
  })
  ...
  it('can remove an attachment (as moderator)', function() {
    ...
    expect(removeBtns.length, 'Initially zero remove attachment buttons')
        .to.equal(0)
    ...
    expect(removeBtns.length, 'One remove attachment buttons after upload')
        .to.equal(1)
    // REMOVE ATTACHMENT!
    removeBtns[0].click()
    ...
    removeBtns = E2EAttachments.getRemoveButtons()
    expect(removeBtns.length, 'Zero remove attachment buttons after remove')
        .to.equal(0)
  })
  ...
  it('has correct UI on finalized minutes with attachments (as moderator)',
     function() {
       E2EAttachments.uploadFile(_staticLocalFilename)
       expect(E2EAttachments.isUploadButtonVisible(),
  ...
  it('can upload an attachment to the server (as uploader)', function() {
    E2EAttachments.switchToUserWithDifferentRole(E2EGlobal.USERROLES.Uploader,
                                                 _projectName, _lastMeetingName)
  ...
  it('can remove only my own attachment (as uploader)', function() {
    ...
    expect(E2EAttachments.getRemoveButtons().length,
    ...
    const attDocAfter =
        E2EAttachments.getAttachmentDocsForMinuteID(_lastMinutesID)
    expect(attDocBefore, '1st upload is still there after remove')
  ...
  it('can not upload but sees download links (as invited)', function() {
    E2EAttachments.uploadFile(_staticLocalFilename)
    E2EAttachments.switchToUserWithDifferentRole(E2EGlobal.USERROLES.Invited,
                                                 _projectName, _lastMeetingName)

    // no need for "expanding"... it is still expanded from user1...
    expect(E2EAttachments.isUploadButtonVisible()).to.be.false
    expect(E2EAttachments.getDownloadLinks().length,
     ...
     function() {
       if (!E2EGlobal.browserIsPhantomJS() &&
         ...
         E2EAttachments.switchToUserWithDifferentRole(
             E2EGlobal.USERROLES.Invited, _projectName, _lastMeetingName)

         const fileShort =
             path.basename(_staticLocalFilename) // => e.g. "favicon.ico"
         ...
         expect(fs.existsSync(downloadTargetFile))
             .to.be.true // File should be there
  ...
  it('has no published attachment if not invited', function() {
    E2EAttachments.uploadFile(_staticLocalFilename)
    expect(E2EAttachments.countAttachmentsOnClientForCurrentUser() > 0,
           'How many attachments are published to the client for user1')
        .to.be.true

    E2EApp.loginUser(2) // switch to non-invited user
    expect(E2EAttachments.countAttachmentsOnClientForCurrentUser(),
