...
describe.skip('Migrations', function() {
  ...
  it('should not change meeting series topics history when migration down and up',
     function() {
       ...
       E2ETopics.addInfoItemToTopic(
           {subject : 'new action item', itemType : 'actionItem'}, 1)
       ...
       const checkHistory =
           () => {
             ...
             expect(topics.length, 'Meeting Series should have one topic')
                 .to.equal(1)
             expect(topics[0].infoItems.length, 'Topic should have four items')
                 .to.equal(4)
           }
