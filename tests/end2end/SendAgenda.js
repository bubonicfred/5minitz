...
describe('Send agenda', function() {
  ...
  it('displays a button send agenda on a new created minute',
     function() { expect(browser.isVisible('#btn_sendAgenda')).to.be.true; });
  ...
  it('sends one email to the participant containing the topic but not the info items',
     function() {
       ...
       E2ETopics.addInfoItemToTopic(
           {subject : infoItemSubject, itemType : 'infoItem'}, 1);
       ...
       expect(sentMail.subject, 'the subject should contain the string Agenda')
           .to.have.string('Agenda');
       expect(sentMail.html,
              'the email should contain the subject of the topic')
           .to.have.string(topicSubject);
       expect(sentMail.html, 'the email should not contain the info item')
