...
describe('Topics Methods Security', function() {
  ...
  it('Moderator can insert a new Topic, not logged in can not insert a topic',
     function() {
       ...
       E2ESecurity.tryAddNewTopic(topicSubject, topicId, min.min_id, 0,
                                  'not logged in user can not insert a topic');

       E2EApp.loginUser();
       expect(E2EApp.isLoggedIn()).to.be.true;
       E2ESecurity.tryAddNewTopic(topicSubject, topicId, min.min_id, 1,
  ...
  it('Moderator can update a Topic, not logged in user can not update a topic',
     function() {
       ...
       E2ESecurity.executeMethod(
           E2ESecurity.addTopic, min.min_id,
           {subject : topicSubject, labels : Array(0), _id : topicId});

       E2EApp.logoutUser();
       expect(E2EApp.isNotLoggedIn()).to.be.true;
       E2ESecurity.tryUpdateTopicSubject(
           newSubject, topicId, min.min_id, topicSubject,
           'not logged in user can not update a topic');

       E2EApp.loginUser();
       expect(E2EApp.isLoggedIn()).to.be.true;
       E2ESecurity.tryUpdateTopicSubject(newSubject, topicId, min.min_id,
                                         newSubject,
  ...
  it('Moderator can delete a Topic, not logged in can not delete a topic',
     function() {
       ...
       E2EApp.logoutUser();
       expect(E2EApp.isNotLoggedIn()).to.be.true;
       E2ESecurity.tryRemoveTopic(topicId, min.min_id, 1,
                                  'not logged in user can not delete a topic');

       E2EApp.loginUser();
       expect(E2EApp.isLoggedIn()).to.be.true;
       E2ESecurity.tryRemoveTopic(topicId, min.min_id, 0,
  ...
  it('Moderator can reopen a topic, not logged in can not reopen a topic',
     function() {
       ...
       E2ESecurity.executeMethod(E2ESecurity.updateTopic, topicId,
                                 {isOpen : false});
       expect((server.call('e2e.getTopics', min.min_id))[0].isOpen)
           .to.equal(false);

       E2ESecurity.executeMethod(E2ESecurity.finalizeMinute, min.min_id);
       E2EGlobal.waitSomeTime();
       expect((server.call('e2e.findMinute', min.min_id)).isFinalized)
           .to.equal(true);

       E2EApp.logoutUser();
       expect(E2EApp.isNotLoggedIn()).to.be.true;
       E2ESecurity.tryReopenTopic(topicId, min.ms_id, false,
...
describe('Topics Publish & Subscribe Security', function() {
  ...
  it('Non-logged in users have no unexpected Topics from MeetingSeries published',
     function() {
       ...
       E2ESecurity.executeMethod(
           E2ESecurity.addTopic, min2.min_id,
           {subject : topicSubject, labels : Array(0), _id : topicId2});
