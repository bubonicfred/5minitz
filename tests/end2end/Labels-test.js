  ...
  describe('Labels for Action- / Info Items', function() {
    ...
    it('changes the labels text in finalized minutes if the label will be renamed',
       function() {
         ...
         expect(visibleText, 'setting the label failed')
             .to.have.string(labelName)
         ...
         expect(visibleText, 'label name should have changed')
             .to.have.string(renamedLabel)
       })
    ...
    it('resets the labels name if editing will be canceled', function() {
      ...
      const labelId = E2EMeetingSeriesEditor.changeLabel(
          labelName, renamedLabel, changedColor, false)
      ...
      const newLabelNameValue =
          browser.getValue(selLabelRow + " [name='labelName']")
      expect(newLabelNameValue, 'label name should be restored')
          .to.equal(labelName)

      const newLabelColorValue =
          browser.getValue(selLabelRow + " [name='labelColor-" + labelId + "']")
      expect(newLabelColorValue, 'label color should be restored')
          .to.not.equal(changedColor)
