...
before(function() {
  ...
  browser.waitForVisible =
      function(selector, timeout = 10000, ...args) {
    try {
  ...
  browser.elementIdClick =
    ...
    try {
      ...
      const id = Math.random().toString(36).substr(2, 5)
      console.log(`browser.elementIdClick() target "${
          args[0]}" not found - see screenshot with ID: ${id}`)
  ...
  if (E2EGlobal.browserIsPhantomJS()) {
    browser.setViewportSize(
        {width : 1024, height : browser.getViewportSize('height')})
  }
...
beforeEach(function() {
  ...
  browser.execute((testName) => {console.log('--- TEST CASE STARTED --- >' +
                                             testName + '<')},
                  testName)
