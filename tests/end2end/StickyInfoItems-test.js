...
describe('Sticky Info Items', function() {
  ...
  beforeEach('make sure test user is logged in, create series and add minutes',
             function() {
               ...
               E2ETopics.addInfoItemToTopic(
                   {subject : getNewInfoItemName(), infoItemType : 'infoItem'},
                   1)
             })
  ...
  it('ensures that sticky-info-items will be presented in the next minute again',
     function() {
       ...
       expect(E2ETopics.countItemsForTopic(1), 'The topic should have one item')
           .to.equal(1)
       const itemsOfNewTopic = E2ETopics.getItemsForTopic(1)
       const stickyInfoItem = itemsOfNewTopic[0].ELEMENT
       expect(browser.elementIdText(stickyInfoItem).value,
  ...
  it('ensures that the sticky-status of an info item in a finalized minute can not be modified',
     function() {
       ...
       E2ETopics.toggleInfoItemStickyState(1, 1)
       expect(E2ETopics.isInfoItemSticky(1, 1),
  ...
  it('can not change the sticky status of info-items on the topics page of the meeting series',
     function() {
       E2ETopics.addInfoItemToTopic(
           {subject : getNewInfoItemName(), infoItemType : 'infoItem'}, 1)
       E2ETopics.toggleInfoItemStickyState(1, 1)
       ...
       expect(E2ETopics.isInfoItemSticky(1, 2),
              'non-sticky item should not have changed state')
           .to.be.false
     })
  ...
  it('ensures that changing the subject of a sticky-info-item also updates the related item located ' +
     ...
     function() {
       ...
       expect(E2ETopics.countItemsForTopic(1), 'topic should have one item')
           .to.equal(1)
