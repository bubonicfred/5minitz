...
describe('User Profile/Password editing', function() {
  ...
  it('Buttons Change Password and Edit Profile are not visible for an LDAP user',
     function() {
       ...
       if (E2EApp.isLoggedIn()) {
         ...
         browser.waitUntil((_) =>
                               !browser.isVisible('#navbar-dlgChangedPassword'))
  ...
  it('User can successfully change his password', function() {
    ...
    browser.waitUntil((_) => !browser.isVisible('#frmDlgChangePassword'),
                      waitUntilTimeout)

    // try ty to log in with new password
    E2EApp.logoutUser()
    expect(E2EApp.isNotLoggedIn()).to.be.true
    E2EApp.loginUserWithCredentials(E2EGlobal.SETTINGS.e2eTestUsers[0],
                                    newPassword, false)
    expect(E2EApp.isLoggedIn()).to.be.true
  ...
  it('User can successefully change his profile', function() {
    ...
    browser.waitUntil((_) => !browser.isVisible('#frmDlgEditProfile'),
                      waitUntilTimeout)
  ...
  it('User profile is not changed, if pressing Cancel', function() {
    ...
    browser.waitUntil((_) =>
                          !E2EUser.checkProfileChanged(longName, email).value,
                      waitUntilTimeout)
  })
  ...
  it('User can save his profile with an empty LongName', function() {
    ...
    E2EGlobal.waitSomeTime()
    E2EUser.editProfile(longName, email)
    browser.waitUntil((_) => E2EUser.checkProfileChanged(longName, email).value,
                      waitUntilTimeout)
  })
