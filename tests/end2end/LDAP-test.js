...
describe('LDAP', function() {
  ...
  xit('import cronjob does not produce errors', function() {
    ...
    expect(numberOfUsersAfterImport)
        .to.be.greaterThan(numberOfUsersWithoutLdap);
  });
  ...
  it('ldap user can NOT reset password', function() {
    ...
    if (browser.isVisible('#at-forgotPwd')) {
      ...
      expect(browser.isVisible('.at-error'), 'Error should be visible')
          .to.be.true;
    } else {
      expect(browser.isVisible('#at-forgotPwd'),
