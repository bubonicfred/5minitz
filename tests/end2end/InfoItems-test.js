...
describe('Info Items', function() {
  ...
  it('can add an info item', function() {
    ...
    E2ETopics.addInfoItemToTopic(
        {subject : infoItemName, itemType : 'infoItem'}, topicIndex)
    ...
    const infoItemExpandElementText =
        browser.elementIdText(infoItemExpandElement).value

    expect(infoItemExpandElementText, 'Info item visible text should match')
  ...
  it('shows security question before deleting info items', function() {
    const infoItemName = getNewAIName()
    E2ETopics.addInfoItemToTopic(
        {subject : infoItemName, itemType : 'infoItem'}, 1)
  ...
  it('can submit an info item by pressing enter in the topic field',
     function() {
       ...
       E2ETopics.insertInfoItemDataIntoDialog(
           {subject : infoItemName, itemType : 'infoItem'})
  ...
  it('can edit an info item', function() {
    ...
    let infoItemExpandElementText =
        browser.elementIdText(infoItemExpandElement).value
    expect(infoItemExpandElementText,
    ...
    infoItemExpandElementText =
        browser.elementIdText(infoItemExpandElement).value
    expect(infoItemExpandElementText, 'New label text should match')
  ...
  it('can edit the 2nd info item after creating three of them', function() {
    ...
    for (let i = 1; i < 4; i++) {
      E2ETopics.addInfoItemToTopic(
          {subject : `Info Item #${i}`, itemType : 'infoItem'}, topicIndex)
      E2EGlobal.waitSomeTime()
      if (i === 2) {
    ...
    E2EGlobal.waitSomeTime()
    E2ETopics.insertInfoItemDataIntoDialog(
        {subject : 'Info Item #2 - changed', itemType : 'infoItem'})
    E2ETopics.submitInfoItemDialog()

    const selector =
        `#topicPanel .well:nth-child(${topicIndex}) .topicInfoItem:nth-child(2)`
    const infoItemExpandElement = browser.element(selector).value.ELEMENT
    const infoItemExpandElementText =
        browser.elementIdText(infoItemExpandElement).value
    expect(infoItemExpandElementText, 'Info Item 2 should be edited correctly')
