...
describe('ActionItems', function() {
  ...
  const getNewMeetingName =
      () => {
        aMeetingCounter++ return aMeetingNameBase + aMeetingCounter
      } const getNewTopicName =
          () => {
            aTopicCounter++ return aTopicNameBase + aTopicCounter
          } const getNewAIName =
              () => { aAICounter++ return aAINameBase + aAICounter }
  ...
  addActionItemToFirstTopic() {
    ...
    E2ETopics.addInfoItemToTopic(
        {subject : actionItemName, itemType : 'actionItem'}, 1)
  ...
  beforeEach('make sure test user is logged in, create series and add minutes',
             function() {
  ...
  it('can add an info item', function() {
    ...
    E2ETopics.addInfoItemToTopic(
        {subject : actionItemName, itemType : 'actionItem'}, topicIndex)

    E2EGlobal.waitSomeTime()

    const selector =
        '#topicPanel .well:nth-child(' + topicIndex + ') #headingOne'
    expect(browser.isVisible(selector), 'Action item should be visible')
        .to.be.true

    const actionItemExpandElement = browser.element(selector).value.ELEMENT
    const actionItemExpandElementText =
        browser.elementIdText(actionItemExpandElement).value

    expect(actionItemExpandElementText, 'Action item visible text should match')
  ...
  it('can edit an existing action item', function() {
    ...
    expect(actionItemExpandElementText, 'AI text should have changed')
  ...
  it('can edit an existing action item after an info item was added',
     function() {
       ...
       const newResponsible =
           E2EGlobal.SETTINGS.e2eTestUsers[1] const actionItemIndex =
               2 // II was inserted before AI!
       ...
       const selector = '#topicPanel .well:nth-child(' + topicIndex +
                        ') .topicInfoItem:nth-child(' + actionItemIndex + ')'
       expect(browser.isVisible(selector),
       ...
       const actionItemExpandElementText =
           browser.elementIdText(actionItemExpandElement).value
       expect(actionItemExpandElementText,
  ...
  it('can add an action item by pressing enter in the topic field', function() {
    ...
    E2ETopics.insertInfoItemDataIntoDialog(
        {subject : actionItemName, itemType : 'actionItem'})
  ...
  it('can add an action item and set the priority field', function() {
    ...
    const actionItemName = getNewAIName()
    E2ETopics.addInfoItemToTopic(
        {subject : actionItemName, priority : 5, itemType : 'actionItem'},
        topicIndex)
  ...
  it('toggles the open-state of the first AI', function() {
    ...
    expect(E2ETopics.isActionItemClosed(1, 1), 'the AI should be closed')
        .to.be.true
  })
  ...
  it('toggles the open-state of the second AI', function() {
    ...
    E2ETopics.addInfoItemToTopic(
        {subject : getNewAIName(), itemType : 'actionItem'}, 1)
    E2ETopics.toggleActionItem(1, 2)

    expect(E2ETopics.isActionItemClosed(1, 2), 'the AI should be closed')
        .to.be.true
  })
  ...
  it('can delete an action item', function() {
    ...
    E2ETopics.addInfoItemToTopic(
        {subject : infoItemName, itemType : 'actionItem'}, topicIndex)
