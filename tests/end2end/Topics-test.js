...
describe('Topics', function() {
  ...
  it('shows security question before deleting a topic', function() {
    ...
    const dialogContentElement =
        browser.element(selectorDialog + ' .modal-body').value.ELEMENT
  ...
  it.skip(
      'can not change the order of topics via drag and drop by clicking anywhere',
      function() {
        E2ETopics.addTopicToMinutes('some topic')
        E2ETopics.addTopicToMinutes('some other topic')
        E2ETopics.addTopicToMinutes('yet another topic')

        browser.dragAndDrop('#topicPanel .well:nth-child(3)',
  ...
  it.skip(
      'can change the order of topics via drag and drop by clicking on the sort icon',
      function() {
        E2ETopics.addTopicToMinutes('some topic')
        E2ETopics.addTopicToMinutes('some other topic')
        E2ETopics.addTopicToMinutes('yet another topic')
        ...
        browser.waitForExist(
            '#topicPanel .well:nth-child(3) .topicDragDropHandle')
  ...
  it.skip('can not change the order of topics on the topics page', function() {
    ...
    const visibleTextBeforeSortAttempt =
        browser.elementIdText(firstElementBeforeSortAttempt).value
    ...
    const visibleTextAfterSortAttempt =
        browser.elementIdText(firstElementAfterSortAttempt).value
    expect(visibleTextAfterSortAttempt).to.have.string('yet another topic')
  })

  it.skip(
      'can not change the order of topics of finalized minutes', function() {
        E2ETopics.addTopicToMinutes('some topic')
        E2ETopics.addTopicToMinutes('some other topic')
        E2ETopics.addTopicToMinutes('yet another topic')
  ...
  it.skip('sorting of topics is persistent', function() {
    ...
    const visibleTextBeforeReload =
        browser.elementIdText(firstElementBeforeReload).value
    ...
    const visibleTextAfterReload =
        browser.elementIdText(firstElementAfterReload).value
  ...
  it('can collapse a topic', function() {
    ...
    E2ETopics.addInfoItemToTopic(
        {subject : 'InfoItem#1', itemType : 'infoItem'}, 1)
    E2ETopics.addTopicToMinutes('topic 2')
    E2ETopics.addInfoItemToTopic(
        {subject : 'InfoItem#2', itemType : 'infoItem'}, 1)
    ...
    const firstVisibleInfoItemText =
        browser.elementIdText(firstVisibleInfoitemId).value
  ...
  it('can close topics', function() {
    ...
    expect(E2ETopics.isTopicClosed(1), 'first topic should be closed')
        .to.be.true
    expect(E2ETopics.isTopicClosed(2), 'second topic should be closed')
        .to.be.true
  })
  ...
  it('is possible to mark topics as recurring persistently', function() {
    ...
    expect(E2ETopics.isTopicRecurring(1),
    ...
    expect(E2ETopics.isTopicRecurring(1), 'topic should be recurring')
        .to.be.true
    expect(E2ETopics.isTopicRecurring(2),
     ...
     function() {
       ...
       const minutesId = parts[parts.length - 1]
           ...
           const topic = {
             ...
             infoItems : [],
             labels : []
           } const aUser = E2EGlobal.SETTINGS.e2eTestUsers[0] const aPassword =
               E2EGlobal.SETTINGS.e2eTestPasswords[0]
  ...
  it('add more (2) labels to topic via textbox', function() {
    ...
    E2ETopics.addTopicToMinutes(topicName + ' #' + labelName1 + ' #' +
                                labelName2)
    E2EGlobal.waitSomeTime(500)

    expect(browser.waitForExist('.topic-labels')).to.be.true
    expect(browser.getText('.topic-labels .label:nth-child(1)'))
        .to.equal(labelName1)
    expect(browser.getText('.topic-labels .label:nth-child(2)'))
        .to.equal(labelName2)
  })
  ...
  it('can add a topic with more (2) labels to minutes at the end of topics list',
     function() {
       ...
       const labelName2 = 'testLabel2'
       E2ETopics.addTopicToMinutes('some topic on top')
       E2ETopics.addTopicToMinutesAtEnd(testTopicName + ' #' + labelName1 +
                                        ' #' + labelName2)
       E2EGlobal.waitSomeTime(500)
  ...
  it('can add a topic with responsible to minutes at the end of topics list',
     function() {
       ...
       const topicHeadingText =
           browser.element('#topicPanel .well:nth-child(2) h3').getText()
