...
export class E2EMeetingSeriesEditor {
  ...
  /**
   * Analyze the user editor table in the DOM and generate a dictionary with its
   content
   *
   * Example result:
   { user1:
   ...
   * @param colNumRole    in which 0-based table column is the role text/ role
   <select>?
  ...
  static getUsersAndRoles(colNumUser, colNumRole, colNumDelete) {
    ...
    // <select>
    let usrRoleSelected = [];
    // ensure we get an array here - even in case only one value returned from
    // getValue()!
    try {
    ...
    for (let rowIndex in elementsUserRows.value) {
      ...
      // array Here we try to find out, if we look at a <select> UI element...
