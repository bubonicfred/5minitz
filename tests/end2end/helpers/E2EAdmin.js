...
export class E2EAdmin {
  static getAdminRoute() { return 'admin'; }
  static selectorMap = {
  ...
  static setShowInactive(showInactive) {
    if (showInactive !== E2EGlobal.isCheckboxSelected(
                             E2EAdmin.selectorMap.chkShowInactiveUsers)) {
      // With material design we can only toggle a checkbox via click on it's
      // label
  ...
  static toggleUserActiveState(index) {
    ...
    let selector = E2EAdmin.selectorMap.btnToggleUserInactive + ':nth-child(' +
                   index + ')';
