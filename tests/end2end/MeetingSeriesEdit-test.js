...
describe('MeetingSeries Editor', function() {
  ...
  before('reload page and reset app', function() {
    E2EGlobal.logTimestamp('Starting test suite: ' +
  ...
  it('can clean up child minutes on deleting meeting series', function() {
    ...
    let countDBMeetingSeriesBefore =
    ...
    expect(server.call('e2e.countMeetingSeriesInMongDB'))
        .to.equal(countDBMeetingSeriesBefore + 1);
    expect(server.call('e2e.countMinutesInMongoDB'))
        .to.equal(countDBMinutesBefore + 1);
    ...
    expect(server.call('e2e.countMeetingSeriesInMongDB'))
        .to.equal(countDBMeetingSeriesBefore);
    expect(server.call('e2e.countMinutesInMongoDB'))
        .to.equal(countDBMinutesBefore);
    expect(E2EMeetingSeries.getMeetingSeriesId(aProjectName, aMeetingName))
        .not.to.be.ok;
  });
  ...
  it('can not save meeting series without project or name', function() {
    ...
    expect(browser.isVisible('#btnMeetingSeriesSave'))
        .to.be.true; // dialog still open!
    ...
    E2EApp.gotoStartPage();
    expect(E2EMeetingSeries.getMeetingSeriesId(aProjectName, aMeetingName))
        .to.be.ok; // prj/name should be unchanged
  });
  ...
  it('can save meeting series with new project name and meeting name',
     function() {
       ...
       expect(E2EMeetingSeries.getMeetingSeriesId(aProjectName, aMeetingName))
           .not.to.be.ok;
       expect(E2EMeetingSeries.getMeetingSeriesId(aNewProjectName,
                                                  aNewMeetingName))
           .to.be.ok;
     });
  ...
  it('can restore fields after close and re-open', function() {
    E2EMeetingSeriesEditor.openMeetingSeriesEditor(aProjectName, aMeetingName);
    browser.setValue('input[id="id_meetingproject"]',
    ...
    E2EMeetingSeriesEditor.openMeetingSeriesEditor(aProjectName, aMeetingName,
                                                   'base', true);
    expect(browser.getValue('input[id="id_meetingproject"]'))
        .to.equal(aProjectName);
    expect(browser.getValue('input[id="id_meetingname"]'))
        .to.equal(aMeetingName);
