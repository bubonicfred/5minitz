...
describe(
    'Topics Delete - Forbid deleting topics which were not created within the current minutes',
    function() {
      ...
      beforeEach(
          'goto start page and make sure test user is logged in', function() {
            ...
            E2ETopics.addInfoItemToTopic(
                {subject : EXISTING_ACTION, itemType : 'actionItem'}, 1);
            E2ETopics.addInfoItemToTopic(
                {subject : EXISTING_STICKY_INFO, itemType : 'infoItem'}, 1);
      ...
      it('is allowed to delete a topic which was created within the current minutes even if it has open action items',
         function() {
           ...
           E2ETopics.addInfoItemToTopic(
               {subject : 'fresh action item', itemType : 'actionItem'}, 1);
      ...
      it('closes the topic together with its open action items instead of deleting it',
         function() {
           E2ETopics.deleteTopic(1, true);
           expect(E2ETopics.countTopicsForMinute()).to.equal(1);
           expect(E2ETopics.isTopicClosed(1), 'the topic should be closed now')
               .to.be.true;
           expect(E2ETopics.isActionItemClosed(1, 2),
      ...
      it('shows a info dialog and does nothing if the topic and its actions are already closed',
         function() {
           ...
           let dialogContentElement =
               browser.element(selectorDialog + ' .modal-header').value.ELEMENT;
           let dialogContentTitle =
      ...
      it('closes the action item instead of deleting it even it was recently edited',
         function() {
           const topicIndex = 1, itemIndex = 2,
           ...
           expect(E2ETopics.isActionItemClosed(topicIndex, 2),
