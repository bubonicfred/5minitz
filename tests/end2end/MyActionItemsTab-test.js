...
describe('MyActionItems Tab', function() {
  ...
  it('can filter my action items from all meeting series', function() {
    ...
    expect(E2ETopics.countItemsForTopic('#itemPanel'),
  ...
  it('can navigate from AI on "My Action Item" to parent topic', function() {
    ...
    const firstAItextOnMyActionItemsView =
        firstActionItemOnMyActionItemsView.getText().replace(/\n.*/, '');
    E2EGlobal.clickWithRetry(
        'a.linkItemContext'); // navigate to parent topic view

    const firstItemOnTopicView = E2ETopics.getNthItemFromItemList(0);

    expect(firstAItextOnMyActionItemsView, 'AI before & AI after shall be same')
        .to.equal(firstItemOnTopicView.getText());
  });
});
