...
describe('Item Details', function() {
  ...
  beforeEach('make sure test user is logged in, create series and add minutes',
             function() {
               ...
               E2ETopics.addInfoItemToTopic(
                   {subject : infoItemName, itemType : 'actionItem'}, 1)
             })
  ...
  it('can add first details to a new Info Item', function() {
    ...
    E2ETopics.addFirstDetailsToNewInfoItem(
        {subject : getNewAIName(), itemType : 'infoItem'}, 1, detailsText)
  ...
  it('can add details to an Info Item, too', function() {
    ...
    E2ETopics.addInfoItemToTopic(
        {subject : getNewAIName(), itemType : 'infoItem'}, 1)
  ...
  it('does not remove details when AI will be updated with the edit dialog',
     function() {
       ...
       expect(completeAIText, 'Subject of AI should have changed')
           .to.have.string(newSubject)
       expect(completeAIText, 'AI should still contain the details')
           .to.have.string(formatDateISO8601(new Date()) + ' New' +
                           '\nNew Details')
     })
  ...
  it('does not revert changes when input field receives click-event during input',
     function() {
       const doBeforeSubmit =
           (inputElement) => {
             // perform click event on the input field after setting the text
             // and before submitting the changes
             E2EGlobal.clickWithRetry(inputElement)
           }

                             E2ETopics.addDetailsToActionItem(
                                 1, 1, 'First Details', doBeforeSubmit)
  ...
  it('shows an confirmation dialog when removing existing details', function() {
    ...
    E2ETopics.editDetailsForActionItem(1, 1, 1,
                                       '') // empty text will remove the detail
  ...
  it('can follow a-hyperlink in details', function() {
    E2ETopics.addDetailsToActionItem(
        1, 1, 'New Details with link to http://www.google.com')
