...
describe('Minutes Finalize', function() {
    ...
    it('asks if emails should be sent before finalizing the minute',
       function() {
         ...
         E2ETopics.addInfoItemToTopic(
             {subject : 'action item', itemType : 'actionItem'}, 1)
         E2ETopics.addInfoItemToTopic(
             {subject : 'info item', itemType : 'infoItem'}, 1)
  ...
  it('can not add minutes if unfinalized minutes exist', function() {
    ...
    expect(browser.isExisting('#btnAddMinutes'),
    ...
    expect(browser.getUrl(), 'Route should not have changed')
        .to.equal(urlBefore)
    expect(E2EMinutes.countMinutesForSeries(aProjectName, aMeetingName),
  ...
  it('can finalize minutes at later timepoint', function() {
    ...
    E2EGlobal.clickWithRetry(
        'a#id_linkToMinutes') // goto first available minutes
    // Now finalize!
    E2EMinutes.finalizeCurrentMinutes()
    E2EMeetingSeries.gotoMeetingSeries(aProjectName, aMeetingName)
  ...
  it('removes all fresh info items when unfinalizing the second minutes',
     function() {
       this.timeout(150000)
       aMeetingCounter++
       aMeetingName = aMeetingNameBase + aMeetingCounter
       ...
       E2ETopics.addInfoItemToTopic(
           {subject : 'Old Info Item', itemType : 'infoItem'}, 1)

       E2EMinutes.finalizeCurrentMinutes()

       E2EMinutes.addMinutesToMeetingSeries(aProjectName, aMeetingName, myDate)
       E2ETopics.addInfoItemToTopic(
           {subject : 'New Info Item', itemType : 'infoItem'}, 1)
       E2EMinutes.finalizeCurrentMinutes()
       ...
       expect(E2ETopics.countItemsForTopic(1), 'Topic should have one items')
           .to.equal(1)
     })
  ...
  it('prohibits unfinalizing of non-latest minutes', function() {
    ...
    E2EMinutes.addMinutesToMeetingSeries(
        aProjectName, aMeetingName, myDate2) // myDate2 is kept UNFINALIZED!

    E2EMeetingSeries.gotoMeetingSeries(aProjectName, aMeetingName)
    E2EMinutes.gotoMinutes(myDate1)
    expect(browser.isExisting('#btn_unfinalizeMinutes')).to.be.false

    E2EMeetingSeries.gotoMeetingSeries(aProjectName,
                                       aMeetingName) // now FINALIZE myDate2
