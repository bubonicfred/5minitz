...
describe('MeetingSeries Items list', function() {
  ...
  it('displays all info- and action-items of all topics', function() {
    ...
    E2ETopics.addInfoItemToTopic(
        {subject : 'some information of another topic'}, 1);
    E2ETopics.addInfoItemToTopic({
    ...
    expect(E2ETopics.getAllItemsFromItemList().length,
    ...
    expect(E2ETopics.getNthItemFromItemList(1).getText(),
           'First item should have correct subject')
        .to.have.string('some information of another topic');
    expect(E2ETopics.getNthItemFromItemList(2).getText(),
           'First item should have correct subject')
        .to.have.string('some action item');
    expect(E2ETopics.getNthItemFromItemList(3).getText(),
           'First item should have correct subject')
        .to.have.string('some information');
  });

  it.only('can expand an info item to display its details on the item list',
          function() {
            E2ETopics.addTopicToMinutes('some topic');
            E2ETopics.addInfoItemToTopic({subject : 'some information'}, 1);
            E2ETopics.addDetailsToActionItem(
                1, 1, 'Amazing details for this information item');
