...
describe("MeetingSeries Items list", function () {
  ...
  before("reload page and reset app", function () {
    E2EGlobal.logTimestamp(
      "Starting test suite: " + E2EGlobal.getTestSpecFilename()
    );
    server.connect();
    E2EApp.resetMyApp();
    E2EApp.launchApp();
  ...
  it("displays all info- and action-items of all topics", function () {
    ...
    expect(
      E2ETopics.getNthItemFromItemList(0).getText(),
      "First item should have correct subject"
    ).to.have.string("some action item of another topic");
    expect(
      E2ETopics.getNthItemFromItemList(1).getText(),
      "First item should have correct subject"
    ).to.have.string("some information of another topic");
    expect(
      E2ETopics.getNthItemFromItemList(2).getText(),
      "First item should have correct subject"
    ).to.have.string("some action item");
    expect(
      E2ETopics.getNthItemFromItemList(3).getText(),
      "First item should have correct subject"
  ...
  it.only("can expand an info item to display its details on the item list", function () {
    ...
    expect(E2ETopics.getNthItemFromItemList(0).getText()).to.have.string(
