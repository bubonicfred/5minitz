...
describe('Protocols', function() {
  ...
  it('No Protocol is created on finalizing Minutes if feature is disabled',
     function() {
       E2EProtocols
           .setSettingsForProtocolGeneration() // Disable document generation

       const numberOfProtocolsBefore = E2EProtocols.countProtocolsInMongoDB()
       E2EMinutes.finalizeCurrentMinutes()

       expect(browser.isExisting('#btn_unfinalizeMinutes'),
              'Minute has been finalized')
           .to.be.true

       expect(E2EProtocols.countProtocolsInMongoDB(),
  ...
  it('HTML Protocol is created on finalizing Minutes', function() {
    ...
    expect(E2EProtocols.checkProtocolFileForMinuteExits(_lastMinutesID),
  ...
  it('Download Button is visible on finalized Minutes', function() {
    ...
    expect(E2EProtocols.downloadButtonExists(),
  ...
  xit('Trying to download a non-existent protocol shows a confirmation dialog to download on-the-fly version',
      function() {
        ...
        E2EProtocols.setSettingsForProtocolGeneration(
            'html') // Reactivate protocol generation, otherwise there won't be
                    // a download-button
        ...
        expect(E2EProtocols.checkDownloadOpensConfirmationDialog(),
  ...
  it('Trying to download an existant protocol shows no confirmation dialog',
     function() {
       ...
       browser.scroll(
           '.navbar-header') // without this the "Minutes finalized" toast would
                             // be right above the download button
