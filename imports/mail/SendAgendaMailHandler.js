...
export class SendAgendaMailHandler extends InfoItemsMailHandler {
  ...
  constructor(sender, minute) {
    super(sender, minute.getPersonsInformedWithEmail(Meteor.users), minute,
          minute.getOpenTopicsWithoutItems(), minute.parentMeetingSeries(),
          minute.getParticipants(Meteor.users),
          minute.getInformed(Meteor.users), 'sendAgenda');
  }
  ...
  _getEmailData() {
    ...
    return {
      ...
      meetingSeriesURL :
          GlobalSettings.getRootUrl('meetingseries/' + this._meetingSeries._id),
      ...
      attachments :
          DocumentGeneration.getAttachmentsFromMinute(this._minute._id),
