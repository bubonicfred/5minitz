      ...
      db.collection('minutes')
          ...
          .then(allMinutesDoc => {
            if (allMinutesDoc) {
              ...
              console.log('Saved: ' + minFile + ' with ' +
                          allMinutesDoc.length + ' minutes');

              // Collect additional invited / informed users from older minutes
              allMinutesDoc.map(min => {
                min.visibleFor &&
                    min.visibleFor.map(
                        userID => { // should be identical to meeting series
                          userIDs[userID] = 1;
                        });
                min.informedUsers &&
                    min.informedUsers.map(
                        userID => { // should be identical to meeting series
                          userIDs[userID] = 1;
                        });
                min.participants &&
                    min.participants.map(
                        part => { // might differ from meeting series users!
                ...
                min.topics && min.topics.map(top => { // iterate topics
                  ...
                  top.infoItems &&
                      top.infoItems.map(
                          item => { // topic-actionitem-responsibles
                            item.responsibles &&
            ...
            return db.collection('minutes')
                ...
                .then(function(res) {
                  if (res.result.ok === 1 && res.result.n === minDoc.length) {
                    console.log('OK, inserted ' + res.result.n +
                                ' meeting minutes.');
                    resolve({db, usrMap});
                  } else {
  ...
  static patchUsers(minDoc, usrMap) {
    ...
    for (let m = 0; m < minDoc.length; m++) { // iterate all minutes
      ...
      for (let t = 0; minDoc[m].topics && t < minDoc[m].topics.length; t++) {
        minDoc[m].topics[t] =
            ExpImpTopics.patchUsers(minDoc[m].topics[t], usrMap);
      }
    }
