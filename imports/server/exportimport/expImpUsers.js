      ...
      db.collection('users')
          ...
          .then(allUsersDoc => {
            if (allUsersDoc) {
              ...
              console.log('Saved: ' + usrFile + ' with ' + allUsersDoc.length +
                          ' users');
          ...
          .then(doc => {
            if (doc) {
              console.log('Found ' + doc.length +
                          ' target users in current user DB.');
              console.log('Will copy over ' + usrCopyIDs.length +
              ...
              db.collection('users')
                  ...
                  .then(shouldBeEmpty => {
                    if (shouldBeEmpty && shouldBeEmpty.length > 0) {
                      ...
                      return reject(shouldBeEmpty.length +
  ...
  static doImport(db, msID, usrMap) {
    return new Promise((resolve, reject) => {
      ...
      for (let u = 0; u < allUsersDoc.length; u++) {
        if (allUsersDoc[u]._id ===
            usrMap[allUsersDoc[u]._id]) { // before/after ID are same in mapping
                                          // file!
          let roleValueForMS =
          ...
          promiseChain.push( // Case#2: only update user role for existing user
                             // in target DB
              db.collection('users')
                  .findOne({
                  ...
                  .then(function(usr) {
                    ...
                    if (roleValueForMS &&
                        roleValueForMS.length >
                            0) { // user needs role for import meeting series?
