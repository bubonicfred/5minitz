...
export class DocumentGeneration {
  ...
  static async downloadMinuteProtocol(minuteID, noProtocolExistsDialog) {
    ...
    let generateAndDownloadHTML = async function() {
      ...
      // Create HTML
      let htmldata =
          await Meteor.callPromise('documentgeneration.createHTML', minuteID);
      ...
      if (window.navigator
              .msSaveOrOpenBlob) { // necessary for Internet Explorer, since it
                                   // does'nt support the download-attribute for
                                   // anchors
  ...
  static generateResponsibleStringsForTopic(context) {
    context._topics.forEach(topic => {
      ...
      topic.labelsString = LabelResolver.resolveAndformatLabelsString(
          topic.labels, context._meetingSeries._id);
  ...
  static getDocumentData(context) {
    ...
    return {
      ...
      meetingSeriesURL : GlobalSettings.getRootUrl('meetingseries/' +
                                                   context._meetingSeries._id),
      minuteUrl :
          GlobalSettings.getRootUrl('minutesedit/' + context._minute._id),
  ...
  static addHelperForHTMLMail(template, context) {
    ...
    template.addHelper('formatLabels', function() {
      return LabelResolver.resolveAndformatLabelsString(
          this.labels, context._minute.parentMeetingSeriesID());
    });
