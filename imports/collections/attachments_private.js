...
export let AttachmentsCollection = new FilesCollection({
  ...
  onBeforeUpload : function(file) {
    ...
    if (file.size > Meteor.settings.public.attachments.maxFileSize) {
      const maxMB = Math.floor(Meteor.settings.public.attachments.maxFileSize /
                               1024 / 1024);
    ...
    if (Meteor.settings.public.attachments.denyExtensions !== undefined) {
      const denyRE = new RegExp(
          '^(' + Meteor.settings.public.attachments.denyExtensions + ')$', 'i');
      const forbiddenRE =
          new RegExp('^(' + FORBIDDEN_FILENAME_EXTENSIONS + ')$', 'i');
    ...
    const allowRE = new RegExp(
        '^(' + Meteor.settings.public.attachments.allowExtensions + ')$', 'i');
  ...
  onAfterUpload : function(file) {
    console.log('Successfully uploaded attachment file: ' + file.name + ' to ' +
                file.path);
...
extendedPublishSubscribeHandler.publishByMeetingSeriesOrMinute(
    'files.attachments.all', AttachmentsCollection, 'meta.parentseries_id',
  ...
  'attachments.remove'(attachmentID) {
    if (Meteor.isServer && attachmentID) {
      ...
      AttachmentsCollection.remove({_id : attachmentID}, function(error) {
        if (error) {
          console.error('File ' + attachmentID +
                        ' wasn\'t removed, error: ' + error.reason);
        } else {
