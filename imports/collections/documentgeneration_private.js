...
export let DocumentsCollection = new FilesCollection({
  ...
  permissions : parseInt('0600', 8), // #Security: make uploaded files "chmod
                                     // 600' only readable for server user
  ...
  onAfterUpload : function(file) {
    console.log('Successfully created protocol: ' + file.name + ' to ' +
                file.path);
...
extendedPublishSubscribeHandler.publishByMeetingSeriesOrMinute(
    'files.protocols.all', DocumentsCollection, 'meta.meetingSeriesId',
    'meta.minuteId');
...
Meteor.methods({
  'documentgeneration.createHTML'(minuteID) {
    ...
    if (!userRoles.isInvitedTo(minute.parentMeetingSeriesID())) {
      throw new Meteor.Error('Cannot download this minute',
    ...
    i18n.runWithLocale(minute.parentMeetingSeries().getMailLanguage(), () => {
      try {
        ...
        let tmplRenderer =
            new TemplateRenderer('publishInfoItems', 'server_templates/email')
                .addData('name', '');
  ...
  'documentgeneration.createAndStoreFile'(minutesId) {
    ...
    if ((Meteor.settings.public.docGeneration.format === 'pdf') ||
      ...
      storeFileFunction = (htmldata, fileName, metaData) => {
        ...
        console.log('Protocol generation to file: ', finalPDFOutputPath,
                    fileName);
    ...
    try {
      let htmldata =
          Meteor.call('documentgeneration.createHTML',
                      minutesObj._id); // this one will run synchronous
      storeFileFunction(htmldata, fileName, metaData);
    } catch (error) {
  ...
  'documentgeneration.removeFile'(minutesId) {
    if (Meteor.isServer) {
      ...
      DocumentsCollection.remove(
          {'meta.minuteId' : minutesId}, function(error) {
            if (error) {
              throw new Meteor.Error('Protocol could not be deleted, error: ' +
                                     error.reason);
            }
          });
    }
