...
function select2search(selectResponsibles, delayTime, freeTextValidator,
                       minuteID, topicOrItem) {
  const minute = new Minutes(minuteID);
  const preparer = new ParticipantsPreparer(minute, topicOrItem, Meteor.users,
                                            freeTextValidator);
  ...
  selectResponsibles.select2({
    ...
    ajax : {
      ...
      transport(params, success, failure) {
        Meteor.call("responsiblesSearch", params.data.q, participants,
                    (err, results) => {
                      if (err) {
                        failure(err);
                        return;
...
export function configureSelect2Responsibles(SelectResponsibleElementID,
                                             topicOrItemDoc, freeTextValidator,
                                             _minutesID, topicOrItem) {
  const selectResponsibles =
  ...
  if (topicOrItemDoc !== undefined) {
    ...
    responsibles.forEach((responsibleId) => {
      ...
      if (responsibleUser) {
        Minutes.formatResponsibles(responsibleUser, "username",
    ...
    Blaze.renderWithData(Template.optionsElement, data,
                         document.getElementById(SelectResponsibleElementID));
  }
