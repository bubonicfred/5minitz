...
export class TopicInfoItemListContext {
  ...
  static createdReadonlyContextForItemsOfDifferentTopicsAndDifferentMinutes(
      items, resolveSeriesForItem, resolveTopicForItem) {
  ...
  static createReadonlyContextForItemsOfDifferentTopics(items,
                                                        meetingSeriesId) {
  ...
  static createContextForItemsOfOneTopic(items, isReadonly, topicParentId,
                                         parentTopicId) {
    return new TopicInfoItemListContext(items, isReadonly, topicParentId,
                                        parentTopicId);
  }
...
Template.topicInfoItemList.onCreated(function() {
  ...
  this.isItemsLimited =
      new ReactiveVar(tmplData.items.length > INITIAL_ITEMS_LIMIT);
...
const updateItemSorting = (evt, ui) => {
  ...
  const topic =
      new Topic(item.attr("data-topic-parent-id"), item.attr("data-parent-id"));
...
const performActionForItem = (evt, tmpl, action) => {
  ...
  const aInfoItem = findInfoItem(context.getSeriesId(infoItem._id),
                                 infoItem.parentTopicId, infoItem._id);
  action(aInfoItem);
};
...
Template.topicInfoItemList.events({
  ...
  "click #btnDelInfoItem"(evt, tmpl) {
    ...
    performActionForItem(evt, tmpl, (item) => {
      const isDeleteAllowed =
          item.isDeleteAllowed(context.getSeriesId(item._infoItemDoc._id));
      ...
      if (item.isSticky() || isDeleteAllowed) {
        ...
        ConfirmationDialogFactory
            .makeWarningDialogWithTemplate(action, title, "confirmDeleteItem",
                                           templateData, button)
            .show();
        return;
      }
      ...
      ConfirmationDialogFactory
          .makeInfoDialog(i18n.__("Dialog.ItemDeleteError.title"),
  ...
  "click .btnConvertInfoItem"(evt, tmpl) {
    ...
    const item = findInfoItem(context.topicParentId, infoItem.parentTopicId,
                              infoItem._id);
    // if edit is allowed topicParentId == currentMinutesId
    if (ItemsConverter.isConversionAllowed(item.getDocument(),
                                           context.topicParentId)) {
      ItemsConverter.convertItem(item).catch(handleError);
    } else {
      ConfirmationDialogFactory
          .makeInfoDialog(i18n.__("Dialog.ConvertItemError.title"),
  ...
  "click .detailText"(evt, tmpl) {
    ...
    if (aActionItem._infoItemDoc.details[detailIndex].isEditedBy !==
            undefined &&
        aActionItem._infoItemDoc.details[detailIndex].isEditedDate !==
            undefined) {
      const unset = () => {
        IsEditedService.removeIsEditedDetail(aMin._id, aTopic._topicDoc._id,
      ...
      ConfirmationDialogFactory
          .makeWarningDialogWithTemplate(
              unset, i18n.__("Dialog.IsEditedHandling.title"),
              "confirmationDialogResetEdit", tmplData,
              i18n.__("Dialog.IsEditedHandling.button"))
          .show();
    } else {
      IsEditedService.setIsEditedDetail(aMin._id, aTopic._topicDoc._id,
                                        aActionItem._infoItemDoc._id,
                                        detailIndex);
    ...
    isEditedHandling(element, unset, setIsEdited, evt,
    ...
    if (text === "" || text !== textEl.attr("data-text")) {
      if (text === "") {
        ...
        if (oldText) {
          ...
          // removed
          ConfirmationDialogFactory
              .makeWarningDialog(deleteDetails, undefined,
