...
Template.topicInfoItemEdit.onCreated(function() {
  ...
  const user = new User();
  this.collapseState =
      new ReactiveVar(user.getSetting(userSettings.showAddDetail, true));
});
...
const toggleItemMode = (type, tmpl) => {
  ...
  case "actionItem":
    ...
    configureSelect2Responsibles("id_selResponsibleActionItem",
                                 editItem._infoItemDoc, freeTextValidator,
                                 _minutesID, editItem);
    break;
  case "infoItem":
...
function closePopupAndUnsetIsEdited() {
  IsEditedService.removeIsEditedInfoItem(
      _minutesID, Session.get("topicInfoItemEditTopicId"),
      Session.get("topicInfoItemEditInfoItemId"), false);
...
Template.topicInfoItemEdit.events({
  async "submit #frmDlgAddInfoItem"(evt, tmpl) {
    ...
    try {
      ...
      if (Session.get("topicInfoItemEditInfoItemId") !== null)
        IsEditedService.removeIsEditedInfoItem(
            _minutesID, Session.get("topicInfoItemEditTopicId"),
            Session.get("topicInfoItemEditInfoItemId"), true);
      const editItem = getEditInfoItem();
      ...
      const newItem = createItem(doc, getRelatedTopic(), _minutesID,
                                 minutes.parentMeetingSeries(), type, labels);
  ...
  "show.bs.modal #dlgAddInfoItem"(evt, tmpl) {
    ...
    configureSelect2Labels(_minutesID, "#id_item_selLabelsActionItem",
                           getEditInfoItem());
    ...
    if (editItem) {
      ...
      configureSelect2Responsibles("id_selResponsibleActionItem",
                                   ...
                                   _minutesID, editItem);
      const selectLabels =
          document.querySelector("#id_item_selLabelsActionItem");
      if (selectLabels) {
        selectLabels.value = "";
      }
  ...
  "select2:selecting #id_selResponsibleActionItem"(evt) {
    if (evt.params.args.data.id === evt.params.args.data.text &&
      ...
      ConfirmationDialogFactory
          .makeInfoDialog(i18n.__("Dialog.ActionItemResponsibleError.title"),
