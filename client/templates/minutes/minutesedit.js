...
let togglePrintView = function(switchOn) {
  if (switchOn === undefined) { // toggle on <=> off
    Session.set('minutesedit.PrintViewActive',
  ...
  if (Session.get('minutesedit.PrintViewActive')) {
    ...
    Session.set('minutesedit.collapsetopics-save4print.' + _minutesID,
...
let updateTopicSorting = function(event, ui) {
  ...
  const oldDragTopicPos =
      minute.topics.findIndex(t => t._id === draggedTopicID);
  const oldFollowerPos =
      minute.topics.findIndex(t => t._id === followerTopicID);
  ...
  if (oldFollowerPos >= 0) {                    // insert before new follower
    ...
    const lastVisibleTopicPos =
        newTopicSorting.findIndex(t => t._id === lastVisibleTopicId);
...
Template.minutesedit.helpers({
  ...
  checkParentSeries : function() {
    ...
    try {
      ...
      orphanFlashMessage =
          (new FlashMessage(i18n.__('FlashMessages.error'),
  ...
  getTopicsListConfig : function() {
    ...
    if (filterClosedTopics.get()) {
      filteredTopics =
          aMin.topics.filter((topic) => (topic.isOpen) && (!topic.isSkipped));
    } else {
      if (!isModerator()) {
  ...
  theProtocol :
      function() { return DocumentGeneration.getProtocolForMinute(_minutesID); }
});
...
Template.minutesedit.events({
  ...
  'click #btn_sendAgenda' : async function(evt, tmpl) {
    ...
    let aMin = new Minutes(_minutesID);
    console.log('Send agenda: ' + aMin._id +
    ...
    let agendaCheckDate = async () => {
      if (aMin.getAgendaSentAt()) {
        ...
        ConfirmationDialogFactory
            .makeSuccessDialog(
                sendAgenda, i18n.__('Dialog.ConfirmSendAgenda.title'),
    ...
    QualityTestRunner.run(QualityTestRunner.TRIGGERS.sendAgenda, aMin,
                          agendaCheckDate);
  },
  ...
  'click #btn_finalizeMinutes' : function(evt, tmpl) {
    ...
    console.log('Finalize minutes: ' + aMin._id +
                ' from series: ' + aMin.meetingSeries_id);

    let doFinalize = function() {
      tmpl.$('#btn_finalizeMinutes').prop('disabled', true);
      let msg = (new FlashMessage(i18n.__('FlashMessages.finalizeProgress1'),
      ...
      Meteor.setTimeout(() => {
        Finalizer.finalize(aMin._id, sendActionItems, sendInformationItems,
                           handleError);
        tmpl.$('#btn_finalizeMinutes').prop('disabled', true);
        (new FlashMessage(i18n.__('FlashMessages.ok'),
                          i18n.__('FlashMessages.finalizeOK'),
                          FlashMessage.TYPES().SUCCESS, 3000))
            .show();
        msg.hideMe();
        toggleTopicSorting();
        Session.set('participants.expand', false);
      ...
      if (GlobalSettings.isEMailDeliveryEnabled()) {
        ConfirmationDialogFactory
            .makeSuccessDialogWithTemplate(
                doFinalize, i18n.__('Dialog.ConfirmFinalizeMinutes.title'),
                'confirmationDialogFinalize', {
                  ...
                  sendInformationItems : (sendInformationItems) ? 'checked'
                                                                : '',
                  mailLanguage : msLanguage.name + '/' + msLanguage.nameNative +
                                     ' (' + msLanguage.code + ')'
                },
    ...
    QualityTestRunner.run(QualityTestRunner.TRIGGERS.finalize, aMin,
                          processFinalize);
  },
  ...
  'click #btn_unfinalizeMinutes' : function(evt) {
    evt.preventDefault();
    let aMin = new Minutes(_minutesID);
    console.log('Un-Finalize minutes: ' + aMin._id +
                ' from series: ' + aMin.meetingSeries_id);
  ...
  'click #btn_deleteMinutes' : function(evt) {
    ...
    ConfirmationDialogFactory
        .makeWarningDialogWithTemplate(
            deleteMinutesCallback, i18n.__('Dialog.ConfirmDelete.title'),
            'confirmationDialogDeleteMinutes', tmplData)
        .show();
  },
  ...
  'click #btn_dynamicallyGenerateProtocol' : function(evt) {
    ...
    let noProtocolExistsDialog = (downloadHTML) => {
      ConfirmationDialogFactory
          .makeSuccessDialogWithTemplate(
              downloadHTML, i18n.__('Dialog.ConfirmGenerateProtocol.title'),
              'confirmPlainText',
    ...
    DocumentGeneration
        .downloadMinuteProtocol(_minutesID, noProtocolExistsDialog)
        .catch(handleError);
  },
